      ...
      <div className="space-y-4">
        {data.ayat.map((item: Ayat) => (
          <Card
            ...
            key={item.nomorAyat}
            className="border-l-4 border-l-gold-ornament-500 shadow-sm hover:shadow-md transition-shadow scroll-mt-64 border-t-0 border-r-0 border-b-0 rounded-r-xl" 
          >
            <CardContent className="p-6">
              <div className="flex justify-between items-start mb-6">
                <VerseBadge number={item.nomorAyat} />
              ...
              <div className="text-sm space-y-2">
                {showLatin && (
                  <p className="text-gold-ornament-600 font-medium tracking-wide">{item.teksLatin}</p>
                )}
                {showTerjemahan && (
